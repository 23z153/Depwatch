...
def remediation_playbook(result: AnalysisResult) -> list[dict[str, Any]]:
    ...
    for component, component_findings in findings.items():
        ...
        for finding in sorted(component_findings, key=lambda f: (-f.score, f.finding_id)):
            ...
            finding_paths = getattr(finding, "paths", []) or []
            for path in finding_paths:
                # Strip 'generic:' prefix for readability
                path_str = " -> ".join(p[8:] if p.startswith("generic:") else p for p in path)
...
function selectSystem(i){active=i;render();show('graphview')}function playbook(p){const list=p.playbook||[];$('#playbook-count').textContent=`${list.length} action item${list.length===1?'':'s'}`;$('#playbook').innerHTML=list.map(x=>`<article class="play"><div class="play-top"><span class="component">${esc(x.component)}</span><span class="pill risk">Risk ${x.risk_score}</span></div><div class="system-meta">${esc(x.findings.join(' · '))}</div>${x.paths && x.paths.length ? `<div class="system-meta" style="margin-top:5px;color:var(--muted)"><strong>Dependency path${x.paths.length > 1 ? 's' : ''}:</strong><br>${x.paths.map(path => `<code>${esc(path)}</code>`).join('<br>')}</div>` : ''}${x.steps.map((s,i)=>`<div class="step"><b>${i+1}</b><span>${esc(s)}</span></div>`).join('')}</article>`).join('')||'<div class="empty">No remediation actions for this system.</div>'}
function graph(p){const svg=$('#graph'),all=[{id:'ROOT',name:'Root',version:'',direct:false},...p.components],by=Object.fromEntries(all.map(n=>[n.id,n])),parents={},allChildren={};p.dependencies.forEach(e=>{(parents[e.to]??=[]).push(e.from);(allChildren[e.from]??=[]).push(e.to);});const risky=new Set((p.vulnerabilities||[]).filter(f=>f.exploitability!=='suppressed').map(f=>f.component));const focused=all.length>70&&risky.size>0,keep=new Set(['ROOT']);if(focused){for(const id of risky){let stack=[id],seen=new Set;while(stack.length){const n=stack.pop();if(seen.has(n))continue;seen.add(n);keep.add(n);(parents[n]||[]).forEach(x=>stack.push(x));}}for(const n of all){if(n.direct){let stk=[n.id],vis=new Set;while(stk.length){const c=stk.pop();if(vis.has(c))continue;vis.add(c);keep.add(c);(allChildren[c]||[]).forEach(x=>stk.push(x));}}}}else all.forEach(n=>keep.add(n.id));const nodes=all.filter(n=>keep.has(n.id)),edges=p.dependencies.filter(e=>keep.has(e.from)&&keep.has(e.to));const children={};edges.forEach(e=>(children[e.from]??=[]).push(e.to));const depth={ROOT:0},queue=['ROOT'];while(queue.length){const from=queue.shift();for(const to of children[from]||[]){const next=depth[from]+1;if(depth[to]===undefined||next<depth[to]){depth[to]=next;queue.push(to)}}}for(const n of nodes)if(depth[n.id]===undefined)depth[n.id]=0;const levels={};nodes.forEach(n=>(levels[depth[n.id]]??=[]).push(n));const maxDepth=Math.max(...Object.values(depth)),maxRows=Math.max(...Object.values(levels).map(x=>x.length)),w=Math.max(900,180+(maxDepth+1)*220),h=Math.max(430,110+maxRows*72),clip=(s,n)=>s.length>n?s.slice(0,n-1)+'…':s,display=n=>{if(n.id==='ROOT')return 'ROOT';let name=n.name||n.id;if(n.ecosystem==='maven'&&name.includes(':'))name=name.split(':').pop();return clip(name,20)};Object.entries(levels).forEach(([d,list])=>list.forEach((n,i)=>{n.x=70+Number(d)*220;n.y=55+(i+1)*h/(list.length+1)}));svg.style.minWidth=w+'px';svg.style.height=h+'px';svg.setAttribute('viewBox',`0 0 ${w} ${h}`);svg.innerHTML='';edges.forEach(e=>{const a=by[e.from],b=by[e.to];if(a&&b)svg.innerHTML+=`<line class="edge" x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}"/>`});nodes.forEach(n=>{const kind=n.id==='ROOT'?'root':n.direct?'direct':'transitive',name=display(n),version=n.id==='ROOT'?'':clip(n.version||'',14),isRisk=risky.has(n.id);svg.innerHTML+=`<g class="node" onclick="showNode('${encodeURIComponent(n.id)}')"><circle class="${kind}" cx="${n.x}" cy="${n.y}" r="${isRisk?16:13}"/><text text-anchor="middle" style="font-weight:${isRisk?750:650}" x="${n.x}" y="${n.y+31}"><tspan x="${n.x}">${esc(name)}</tspan>${version?`<tspan x="${n.x}" dy="13" style="font-size:10px;fill:#667085;font-weight:500">${esc(version)}</tspan>`:''}</text></g>`});const project=p.project.split('/').filter(Boolean).pop()||p.project;$('#graph-title').textContent=`${focused?'Risk-highlighted':'Full'} dependency graph · ${project}`;$('#graph-count').textContent=`${nodes.length} component${nodes.length===1?'':'s'}`}
function showNode(id){id=decodeURIComponent(id);const p=data.projects[active],item=(p.playbook||[]).find(x=>x.component===id);const parents={};p.dependencies.forEach(e=>{(parents[e.to]??=[]).push(e.from)});const paths=[];function walk(node,currentPath){if(node==='ROOT'){paths.push(['ROOT',...currentPath]);return}const nodeParents=parents[node]||[];for(const pr of nodeParents){if(!currentPath.includes(pr)){walk(pr,[node,...currentPath])}}}walk(id,[]);const formattedPaths=paths.map(path=>path.map(n=>n.startsWith('generic:')?n.slice(8):n).join(' -> '));let html=`<b>${esc(id)}</b>`;if(formattedPaths.length>0){html+=`<div class="system-meta" style="margin-top:6px;margin-bottom:10px"><strong>Dependency path${formattedPaths.length>1?'s':''}:</strong><br>${formattedPaths.map(path=>`<code>${esc(path)}</code>`).join('<br>')}</div>`}if(item){html+=item.steps.map((s,i)=>`<div class="step"><b>${i+1}</b><span>${esc(s)}</span></div>`).join('')}else{html+=`<span class="system-meta">No active remediation action for this component.</span>`}$('#selected').innerHTML=html}load();setInterval(load,5000);
</script></body></html>'''
...
(() => {
  ...
  note.textContent = 'The graph shows the full transitive dependency tree. Vulnerable components appear as larger nodes. On very large graphs, use search (/) to jump straight to any component.';
